...
export function $set_design_label(newLabelDesign: Partial<LabelDesign>) {
...
export function $set_design_description(
...
export function $set_design_element(newElementDesign: Partial<ElementDesign>) {
  const forms = $all_forms.get();
  const workspaceId = selectedWorkspaceId.get();
  const formId = selectedFormId.get();
